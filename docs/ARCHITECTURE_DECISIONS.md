...
Previously OUTCOME JSON supplied `now`; Phase 10F consumes an injected number but defines no clock provenance itself. `createAgentTransitionEvaluator` now accepts a runtime-owned clock dependency (default Date.now), sampled once per requested terminal edge. OUTCOME rejects a caller `now` property. A safe, nonnegative integer clock observation is passed to unchanged 10F logic; non-finite, fractional, negative, unsafe, or throwing clocks fail closed. Tests inject deterministic clocks. ATTEMPT/RECEIPT retain their existing time field for compatibility, but it cannot prove EXPIRED, which accepts only OUTCOME. Production trusts the runtime wall clock, not a remote timestamp or JSON; clock synchronization is outside this bounded change.
