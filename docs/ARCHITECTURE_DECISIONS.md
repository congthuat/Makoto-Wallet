...
## ADR-030 - Phase 12F status presentation boundary
The Agent action workspace uses a read-only status surface. Its current production drafts have no canonical Phase 12 transaction state, so the surface says transaction status is unavailable rather than treating an Agent draft or handoff as a Phase 12 PREPARED transaction. The deterministic presenter accepts a validated 12A REQUESTED state, a guarded 12B PLAN_READY result, or a 12C HISTORICAL restore for descriptive display. It never promotes a bare structural transaction state, receipt reference, hash, proposed 12E transition, or historical restore into current confirmed status. Historical display is explicitly labeled and has no action controls. Pending and unknown 12E recovery evaluations stay nonterminal. Source-chain CCTP status is labeled source-only; destination completion remains unverified. There is still no production PlannerPlan-to-Strategy binding or Phase 12 transaction execution caller. The presenter neither restores storage nor calls policy, RPC, model, signer, wallet, submission, retry, or transition APIs. Existing Phase 9, Phase 10F, and wallet signing authority remain unchanged.

12F adversarial review narrowed the plain-object guarded-result presentation path: without a production caller carrying authenticated canonical evidence, it may display PLAN_READY but cannot display a current transaction status, including SUCCESS. Transaction statuses from 12C restore are explicitly prefixed as recorded history, use a historical-only explanation, and do not display the saved hash as fresh evidence. Exact outer and nested presentation wrappers reject extra fields, accessors, proxies that throw, and unsupported runtime values. A future current transaction surface requires its own canonical caller/evidence boundary; this review does not create one.
