...
`PlannerPlan` holds one or more `PlannerIntent` user goals and explicit intent-ID dependencies. An ACTION has one goal and no dependency; a STRATEGY has at least two goals and a dependency edge. Array order does not imply execution order. `PlannerIntent` remains one resolved goal, while Phase 10 `Strategy` owns technical execution steps such as approval, receipt waits, and revalidation. The provider-agnostic plan port accepts only bounded text with canonical 11B ACTION or STRATEGY classification. Its server-only OpenAI Responses adapter has no public HTTP route. Model output is untrusted until both deterministic `PlannerPlan` graph validation and reused 11A `PlannerIntent` validation accept it. A valid plan describes structural support, not transaction safety: Phase 9 remains safety authority and the user wallet remains final signing authority. 11C does not compile a Phase 10 Strategy, prepare or execute transactions, or add Phase 12 autonomy.
