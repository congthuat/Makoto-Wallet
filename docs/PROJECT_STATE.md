...
- **Sub-phase:** 8F — Agent Integration
- **Status:** PENDING REVIEW
- **Next:** 8F — Agent Integration review
...
- **Phase 8F IMPLEMENTED — PENDING REVIEW.** The Agent now routes supported Send, Xylo Swap, and Direct CCTP requests through canonical READ/QUOTE/PREPARE boundaries and their Phase 8E validation. Existing wallet review handoffs support USDC/EURC/cirBTC Send and external/local Xylo Swap. Direct CCTP remains prepared data only because its current wallet flow has no route-specific handoff; App Kit canonical preparation remains unsupported. No signer or submission authority was added. Verification: 6/6 new focused integration tests; 1226/1226 full frontend tests; typecheck PASS; lint 0 errors / 7 inherited warnings; production build PASS; git diff --check PASS. Phase 8G has not started.
