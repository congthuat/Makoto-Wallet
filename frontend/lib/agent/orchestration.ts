...
import type { AgentActionDraft, AgentIntent, AgentPreparationField, AgentPreparationInput, AgentRequestMode, AgentTopic } from "./types.ts";
...
export function routeAgentRequest(intent: AgentIntent): AgentOrchestrationDecision {
  ...
  if (intent.kind === "prepare-action") {
    ...
    if (!input || missingFields.length || conflictingPreparation(input)) return decision(input ? actionTopic(input.kind) : "unknown", "clarification", "clarification", false, false, "missing-details", missingFields);
...
export function missingPreparation(input: AgentPreparationInput): readonly AgentPreparationField[] {
  ...
  if (input.kind === "bridge" && !input.sourceChainId) missing.push("sourceChain");
...
function conflictingPreparation(input: AgentPreparationInput) {
  return Boolean(input.invalidRecipient || input.recipient?.toLowerCase() === "0x0000000000000000000000000000000000000000" || input.kind === "swap" && input.assetId && input.assetId === input.outputAssetId || input.kind === "bridge" && input.sourceChainId && input.sourceChainId === input.destinationChainId);
}
