...
export async function runPrepareTool(context: PrepareContext, request: PrepareRequest): Promise<PrepareResult> {
  ...
  if (request.tool === "send.prepare") {
    ...
    const handoff = makeHandoff({ version: 1, mode: "prepare-only", rawUserText: "", executionEnabled: false, kind: "send", asset: asset.symbol as "USDC" | "EURC" | "cirBTC", amount: formatUnits(request.amount, asset.decimals), recipient: request.recipient, sourceChain: "Arc Testnet" }, request.account, observedAt, expiry);
  ...
  if (request.tool === "swap.prepare") {
    ...
    const handoff = request.slippage === 0.005 ? makeHandoff({ version: 1, mode: "prepare-only", rawUserText: "", executionEnabled: false, kind: "swap", inputAsset: asset.symbol as "USDC" | "EURC", outputAsset: getAssetById(request.outputAsset)!.symbol as "USDC" | "EURC", amount: formatUnits(request.amount, asset.decimals), slippage: 0.005, sourceChain: "Arc Testnet" }, request.account, observedAt, expiry) : undefined;
