...
export function parseActionDraft(raw: string, text = normalize(raw)): AgentActionDraft | undefined {
  ...
  if (has(text, ["send", "gửi"])) kind = "send";
  else if (has(text, ["swap", "đổi", "hoán đổi"])) kind = "swap";
  else if (has(text, ["bridge", "chuyển chuỗi", "chuyển sang base"])) kind = "bridge";
  else if (has(text, ["deposit", "nạp vào vault", "gửi vào vault"])) kind = "vault-deposit";
  else if (has(text, ["withdraw", "rút khỏi vault", "rút vault"])) kind = "vault-withdraw";
  if (!kind) return undefined;
  const amount = text.match(AMOUNT)?.[1]?.replace(",", ".");
  const assets = [...text.matchAll(/\b(usdc|eurc)\b/g)].map((match) => match[1].toUpperCase());
  const recipient = raw.match(ADDRESS)?.[0];
  const destination = text.match(/(?:to|sang|đến)\s+(base sepolia|arc testnet|base)/)?.[1];
  ...
  return Object.freeze({ kind, asset: assets[0] ?? "USDC", amount, recipient, sourceChain: "Arc Testnet", destinationChain: titleChain(destination), outputAsset: assets[1], rawUserText: raw, missingFields: Object.freeze(missingFields), executionEnabled: false });
}
...
function titleChain(value?: string) { if (!value) return undefined; return value.split(" ").map((part) => part[0].toUpperCase() + part.slice(1)).join(" "); }
