...
test("reviewed calldata and deadline are frozen across elapsed time", () => { const quote = createXyloQuote("usdc", "eurc", 1_000_000n, 900_000n, 10_000), prepared = prepareXyloSwapRequest(quote, 0.005, owner, 20_000), later = prepareXyloSwapRequest(quote, 0.005, owner, 30_000); assert.equal(prepared.deadline, 320n); assert.equal(later.deadline, 330n); assert.notEqual(prepared.calldata, later.calldata); assert.equal(prepared.deadline, 320n); assert.equal(validatePreparedXyloSwap(prepared, 900_000n, 40_000).valid, true); });
test("live output above or equal to reviewed minimum keeps the frozen request valid", () => { const prepared = prepareXyloSwapRequest(createXyloQuote("usdc", "eurc", 1_000_000n, 900_000n, 10_000), 0.005, owner, 20_000); const calldata = prepared.calldata, minimum = prepared.minimumReceive; assert.equal(validatePreparedXyloSwap(prepared, minimum, 30_000).valid, true); assert.equal(validatePreparedXyloSwap(prepared, minimum + 50_000n, 30_000).valid, true); assert.equal(prepared.minimumReceive, minimum); assert.equal(prepared.calldata, calldata); });
test("live output below reviewed minimum is blocked", () => { const prepared = prepareXyloSwapRequest(createXyloQuote("usdc", "eurc", 1_000_000n, 900_000n, 10_000), 0.005, owner, 20_000); assert.deepEqual(validatePreparedXyloSwap(prepared, prepared.minimumReceive - 1n, 30_000), { valid: false, reason: "output-below-minimum" }); });
test("reviewed deadline expiry is blocked", () => { const prepared = prepareXyloSwapRequest(createXyloQuote("usdc", "eurc", 1_000_000n, 900_000n, 10_000), 0.005, owner, 20_000); assert.deepEqual(validatePreparedXyloSwap(prepared, prepared.minimumReceive, 320_000), { valid: false, reason: "deadline-expired" }); });
