...
test("classic dashboard fragments remain while Activity stays anchored to the real Overview history", () => {
...
test("Settings stays compact and Help exposes only real support resources", () => {
  ...
  assert.match(settings, /Wallet identity[\s\S]*Network safety[\s\S]*Privacy[\s\S]*Appearance[\s\S]*Language/);
  ...
  for (const topic of ["Wallet & security", "Send / Receive", "Swap", "Bridge", "Agent", "Troubleshooting"]) assert.match(settings, new RegExp(topic.replace("/", "\\/")));
  assert.match(settings, /Help improve Makoto Wallet\./);
  assert.match(settings, /https:\/\/docs\.google\.com\/forms\/d\/e\/1FAIpQLSfH_cQv0Gkxy604YcpVHpitSfoWbF5_ud3f5WG_Jc4d7A6nVg\/viewform/);
...
test("Settings Security and Help fragment navigation expose active aria-current destinations", () => {
  ...
  assert.match(header, /href === "\/settings#security" && !hash/);
...
test("narrow-phone navigation keeps classic destinations and utility links labeled", () => {
  ...
  assert.match(headerCss, /@media\s*\(max-width:\s*767px\)[\s\S]*\.primaryNav\s*\{[^}]*grid-template-columns/);
  assert.match(header, /<a className=\{styles\.navLink\} href="https:\/\/docs\.google\.com\/forms/);
  assert.match(headerCss, /\.navLink\s*\{[^}]*min-height:\s*44px/);
});
