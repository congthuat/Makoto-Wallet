...
test("7 swap gas is estimated from the fresh post-approval quote", () => assert.match(flow, /freshQuote = createXyloQuote[\s\S]*estimateSwapFee\(freshQuote\)/));
...
test("15 MAX uses swap gas only with sufficient allowance", () => { assert.equal(safeMaxCanUseSwapEstimate(9n, 10n), false); assert.equal(safeMaxCanUseSwapEstimate(10n, 10n), true); assert.match(flow, /Approve the token before calculating a safe MAX/); });
