...
test("outdated Agent preview-only copy is removed", () => { const formatter = readFileSync(new URL("./agent/formatter.ts", import.meta.url), "utf8"); assert.doesNotMatch(formatter, /preview-only|Transaction execution by Makoto Agent is not enabled/); assert.match(formatter, /agent\.response\.draftReady/); });
