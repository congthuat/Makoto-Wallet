  ...
  try {
    ...
    for (const width of [390, 900, 1440]) for (const locale of ["en", "vi"]) for (const theme of ["light", "dark"]) {
      ...
      for (const scenario of ["read","fresh","account","chain","prepare","ready","unavailable","insufficient","result","invalid","history","empty"]) {
        ...
        if (scenario==='fresh') {
          run('focus','#agent-question'); run('press','Tab');
          check(`keyboard and visible focus ${label}`,()=>{assert.equal(evaluate(`document.activeElement.tagName`),'SUMMARY');assert.equal(evaluate(`getComputedStyle(document.activeElement).outlineStyle`),'solid');});
          run('press','Tab');check(`collapsed suggestions inert ${label}`,()=>assert.equal(evaluate(`document.activeElement.className`),'prepareButton'));
          run('press','Tab');check(`history clear reachable ${label}`,()=>assert.equal(evaluate(`document.activeElement.closest('.history')!==null`),true));
        }
