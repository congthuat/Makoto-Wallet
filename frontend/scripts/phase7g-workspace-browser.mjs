...
if (process.argv.includes("--serve")) {
  ...
  writeFileSync(path.join(output, "entry.tsx"), `import * as React from "react"; import {createRoot} from "react-dom/client"; import {Fixture,StatusFixture} from "./WorkspaceFixture"; const root=createRoot(document.querySelector('#fixture')); let revision=0; function Mounted({options,version}){React.useEffect(()=>{window.fixtureVersion=version;},[version]);return <Fixture options={options}/>;} window.mountWorkspace=(o={})=>{window.fixtureConnection={address:o.account??(['account','prepare'].includes(o.scenario)?${JSON.stringify(accountB)}:${JSON.stringify(accountA)})};window.fixtureChain=o.chainId??(o.scenario==='chain'?84532:${arc});window.fixturePushed='';sessionStorage.clear();const version=++revision;root.render(<Mounted key={version} options={o} version={version}/>);return version;}; window.mountStatus=(input,locale='en')=>{const version=++revision;root.render(<StatusFixture input={input} locale={locale}/>);window.fixtureVersion=version;return version;}; window.mountWorkspace();`);
