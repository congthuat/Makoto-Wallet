...
type HeaderIconName = "wallet" | "history" | "spark" | "help" | "activity" | "settings" | "network" | "language" | "theme" | "sun" | "address" | "faucet";
const navItems: ReadonlyArray<{ href: string; icon: HeaderIconName; en: string; vi: string }> = [
  ...
  { href: "/#activity", icon: "history", en: "Activity", vi: "Hoạt động" },
  { href: "/agent", icon: "spark", en: "Agent", vi: "Trợ lý" },
  { href: "/settings#security", icon: "settings", en: "Settings", vi: "Cài đặt" },
  { href: "/settings#help", icon: "help", en: "Help & Support", vi: "Trợ giúp" },
];
...
export function AppHeader() {
  ...
  const faucetAvailable = connected && chain.isArc && chain.providerChainId === arcTestnet.id;
  const networkLabel = !connected ? "Arc Testnet"
    : chain.isArc ? "Arc Testnet"
    : chain.providerChainId === undefined ? (locale === "vi" ? "Đang kiểm tra mạng" : "Checking network")
  ...
  return <header className={`${styles.header} ${connected ? styles.connectedHeader : styles.disconnectedHeader}`} data-connected={connected}>
    <aside className={styles.sidebar}>
      <Link className={styles.brand} href="/" aria-label="Makoto Wallet"><span className={styles.brandWords}><strong>MAKOTO</strong><small>WALLET</small></span></Link>
      ...
      <a className={styles.feedbackLink} href="https://docs.google.com/forms/d/e/1FAIpQLSfH_cQv0Gkxy604YcpVHpitSfoWbF5_ud3f5WG_Jc4d7A6nVg/viewform" target="_blank" rel="noopener noreferrer">{locale === "vi" ? "Phản hồi" : "Feedback"}<span aria-hidden="true">↗</span></a>
      <div className={styles.version}>ARC TESTNET <span>v{version}</span></div>
    </aside>
    <div className={styles.topRow}>
      <Link className={styles.topBrand} href="/" aria-label="Makoto Wallet"><span className={styles.brandWords}><strong>MAKOTO</strong><small>WALLET</small></span></Link>
      <span className={styles.networkPill} role="status"><i aria-hidden="true" data-verified={connected && chain.isArc} />{networkLabel}</span>
      <Link href="/agent" className={styles.agentShortcut}><HeaderIcon name="activity" className={styles.pillGlyph} /><span>{locale === "vi" ? "Hỏi Makoto. Hiểu ví của bạn." : "Ask Makoto. Understand your wallet."}</span><span aria-hidden="true">↗</span></Link>
