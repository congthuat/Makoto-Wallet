...
export function RealSwapFlow({ locale, onBusyChange, onConfirmed }: Props) {
  ...
  function invalidate() { setQuote(undefined); setReviewStage(undefined); setError(undefined); setQuickFeedback(undefined); setApprovalGasFee(undefined); setSwapGasFee(undefined); setGasUnavailable(false); }
  ...
  async function estimateSwapFee(candidate: SwapQuote) {
    if (!client || !connection.address) return undefined;
  ...
  async function chooseQuickAmount(percent: SwapQuickPercent) {
    ...
    if (percent === 100 && from.id === "usdc" && selected > 0n && client && connection.address && await chain.verifyNow()) {
      ...
      try {
        ...
        const fee = await estimateSwapFee(createXyloQuote(from.id, to.id, selected, output)), maximum = fee === undefined ? undefined : maxUsdcSwapAfterArcFee(balance, fee);
  ...
  async function review() {
    ...
    try {
      ...
      setQuote(nextQuote);
      try { if (needsApproval) setApprovalGasFee(await estimateApprovalFee(parsed)); else setSwapGasFee(await estimateSwapFee(nextQuote)); } catch { setGasUnavailable(true); }
