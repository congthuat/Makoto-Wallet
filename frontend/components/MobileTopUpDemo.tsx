...
export function MobileTopUpDemo() {
  ...
  return <main className={`${walletStyles.page} ${styles.page}`} data-demo-state={step}><div className={styles.shell}><AppHeader />
    <div className={styles.topupBack}><Link href="/pay">← {t("pay.backToPay")}</Link></div>
    <section className={styles.topupHero} data-demo-state="prototype"><div><span className={styles.badge}>{t("pay.productDemo")}</span><p className={styles.eyebrow}>MAKOTO PAY</p><h1>{t("pay.topup.title")}</h1><p>{t("pay.topup.subtitle")}</p></div><Image src="/makoto/pay/mobile-topup.svg" alt="" width={150} height={150} loading="eager" /></section>
    {step === "entry" && <section className={styles.flowCard} aria-labelledby="topup-entry-title" data-demo-step="entry"><h2 id="topup-entry-title">{t("pay.topup.details")}</h2>
