...
export function ActionDraftCard({ draft, vi }: { draft: AgentActionDraft; vi: boolean }) {
  ...
  return <section className={styles.draft} aria-label={vi ? "Bản nháp hành động" : "Action draft"}><header><strong>{vi ? "Bản nháp hành động" : "Action draft"}</strong><span>{validation.valid ? (vi ? "Sẵn sàng chuẩn bị" : "Ready to prepare") : validation.missingFields.length ? (vi ? "Thiếu thông tin" : "Missing information") : (vi ? "Đã chặn" : "Blocked")}</span></header><dl><div><dt>{vi ? "Hành động" : "Action"}</dt><dd>{labels[draft.kind]}</dd></div><div><dt>{vi ? "Số tiền" : "Amount"}</dt><dd>{`${draft.amount} ${asset}`}{outputAsset ? ` → ${outputAsset}` : ""}</dd></div>{recipient && <div><dt>{vi ? "Người nhận" : "Recipient"}</dt><dd className={styles.longValue}>{recipient}</dd></div>}<div><dt>{vi ? "Mạng" : "Network"}</dt><dd>{sourceChain}{destinationChain ? ` → ${destinationChain}` : ""}</dd></div>{validation.missingFields.length > 0 && <div><dt>{vi ? "Còn thiếu" : "Missing"}</dt><dd>{validation.missingFields.join(", ")}</dd></div>}{validation.errors.length > 0 && <div><dt>{vi ? "Đã chặn" : "Blocked"}</dt><dd>{validation.errors.some((value) => value.startsWith("MAX")) ? (vi ? "Hành động MAX cần dùng luồng thủ công." : "MAX actions require the manual flow.") : validation.errors.join(", ")}</dd></div>}</dl><p>{vi ? "Chuẩn bị không mở ví. Sau đó bạn vẫn phải xem xét an toàn và chọn Tiếp tục đến ví." : "Preparing does not open your wallet. You must still review safety and choose Continue to wallet."}</p><button type="button" className={styles.prepareButton} onClick={prepare} disabled={!validation.valid} aria-describedby={!validation.valid ? helpId : undefined}>{vi ? "Chuẩn bị an toàn" : "Prepare safely"}</button>{!validation.valid && <small id={helpId}>{vi ? "Hoàn tất hoặc sửa thông tin bị chặn trước." : "Complete or correct the blocked information first."}</small>}</section>;
}
