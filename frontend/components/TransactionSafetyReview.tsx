...
export function TransactionExpectedChanges({ intent }: { intent: TransactionIntent }) { const { locale } = usePreferences(), vi = locale === "vi", changes = expectedTransactionChanges(intent); if (!changes.length) return <section aria-labelledby="expected-transaction-changes"><h4 id="expected-transaction-changes">{vi ? "Thay đổi dự kiến" : "Expected changes"}</h4><p className="wallet-notice">{vi ? "Không chuyển token trong giao dịch này." : "No token transfer in this transaction."}</p></section>; return <section aria-labelledby="expected-transaction-changes"><h4 id="expected-transaction-changes">{vi ? "Thay đổi dự kiến" : "Expected changes"}</h4><dl className="wallet-review">{changes.map((change, index) => { const asset = getAssetById(change.assetId)!; return <div key={`${change.assetId}-${change.direction}-${index}`}><dt>{asset.symbol}</dt><dd>{qualifier(change.qualifier, vi)} · {change.direction === "increase" ? "+" : "-"}{formatAssetAmount(change.amount, asset)} {asset.symbol}</dd></div>; })}</dl></section>; }
