...
export async function GET(request: NextRequest) {
  if (request.nextUrl.searchParams.has("url") || [...request.nextUrl.searchParams.keys()].some((key) => key !== "source")) return NextResponse.json({ error: "Only an approved source ID is accepted." }, { status: 400 });
  ...
  const cached = responseCache.get(source.id), now = Date.now();
  ...
  try {
    const value = await retrieveOfficialSource(source, fetch, now);
    responseCache.set(source.id, { value, expiresAt: now + source.ttlSeconds * 1000 });
